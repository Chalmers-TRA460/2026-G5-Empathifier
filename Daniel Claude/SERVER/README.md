...
A prototype tool that turns a clinician's brief shorthand notes into a complete, warm patient-portal reply (1177 / Vårdcentral context). The goal is **time-saving as the adoption mechanism, with empathy as a built-in byproduct**.
...
`.env` may be a hidden file on your computer. Look up how to enable hidden files. MAC COMMAND + SHIFT + . 
...
The small **Dev** pill in the modal header (top right) toggles the clinical sanity checker on/off per-request. Green dot = on. The choice persists per browser. Turn it off to save tokens once you trust the notes you're testing. May need to refresh browser if it doesn't start/ turn off. 
